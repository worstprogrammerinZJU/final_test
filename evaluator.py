#!/usr/bin/env python3
"""
evaluator.py - 专门为 _func0 函数设计的测试脚本
配合 generated_asm/problemX.s 和 human-eval-v2-20210705.jsonl
"""
...
def parse_simple_test_cases(test_code: str) -> List[Tuple[str, str]]:
    """
    简单解析测试用例
    格式: assert candidate([1.0, 2.0, 3.9], 0.3) == True
    """
    test_cases = []
...
def parse_array_string(arr_str: str) -> Tuple[str, int]:
    """解析数组字符串 [1.0, 2.0] -> C数组和长度"""
...
def generate_test_code(func_name: str, test_cases: List[Tuple[str, str]]) -> Optional[str]:
    """生成C测试代码"""
    if not test_cases:
        return None
    ...
    for i, (args_str, expected) in enumerate(test_cases):
        ...
        if len(args_list) < 2:
            print(f"  Warning: Test {i} has insufficient arguments: {args_str}")
            continue
        
        # 解析数组
        array_code, array_len = parse_array_string(args_list[0])
        
        # 解析阈值
        threshold = parse_number_string(args_list[1])
...
int main() {{
    ...
    printf("Testing {func_name} with {len(test_cases)} test cases\\n");
    printf("==================================================\\n");
...
def test_one_task(task_num: int, asm_file: str, tasks_data: Dict) -> Tuple[str, Optional[str], List[str]]:
    ...
    test_cases = parse_simple_test_cases(task.get('test', ''))
    if not test_cases:
        debug_info.append(f"Test code: {task.get('test', '')[:100]}...")
        return "no_test_cases", func_name, debug_info
    
    debug_info.append(f"Found {len(test_cases)} test cases")
    
    # 生成测试代码
    c_code = generate_test_code(func_name, test_cases)
    if not c_code:
    ...
    if compile_result.returncode != 0:
        os.unlink(c_file)
        debug_info.append(f"Compile failed: {compile_result.stderr[:200]}")
...
def main():
    ...
    for asm_num, asm_file in asm_files:
        ...
        if task_result == "passed":
            stats["passed"] += 1
            print(f"  ✅ Task {asm_num}: PASSED")
        elif task_result == "failed":
            stats["failed"] += 1
            print(f"  ❌ Task {asm_num}: FAILED")
        elif task_result == "compile_error":
            stats["compile_error"] += 1
            print(f"  🔨 Task {asm_num}: COMPILE ERROR")
        elif task_result == "timeout":
            stats["timeout"] += 1
            print(f"  ⏰ Task {asm_num}: TIMEOUT")
        elif task_result == "crash":
            stats["crash"] += 1
            print(f"  💥 Task {asm_num}: CRASH")
        else:
            stats["other"] += 1
            print(f"  ⚠️ Task {asm_num}: {task_result.upper()}")
        ...
        if not batch_mode and stats["total"] < len(asm_files):
